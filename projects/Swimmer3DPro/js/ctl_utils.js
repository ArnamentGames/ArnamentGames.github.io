...
function playSound(szSound, iVolume, iLoop) {
    if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
        var oPointer = createjs.Sound.play(szSound, {
            loop: iLoop,
            volume: iVolume
        });
        return oPointer;
...
function sizeHandler() {
    ...
    if (s_oHealtBar !== null) {
        s_oHealtBar.refreshButtonPos(s_iOffsetX, s_iOffsetY);
    }
    if (s_oMobileControls !== null) {
        s_oMobileControls.refreshButtonPos(s_iOffsetX, s_iOffsetY);
    }
    if (s_oModeSelect !== null) {
        s_oModeSelect.refreshButtonPos(s_iOffsetX, s_iOffsetY);
    }
    if (s_oLevelMenu !== null) {
        s_oLevelMenu.refreshButtonPos(s_iOffsetX, s_iOffsetY);
    }
    if (s_oSelectTeam !== null) {
        s_oSelectTeam.refreshButtonPos(s_iOffsetX, s_iOffsetY);
    }
...
function formatTime(iTime) {
    ...
    var iMill = Math.floor((iTime * 1000) % 1000);
    var szMill = ":" + iMill;
    if (iMill < 10) {
        szMill = ":00" + iMill;
    } else if (iMill < 100) {
